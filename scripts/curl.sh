...
curl -X POST --data-binary @test/resources/inputs/test-cifar.png -H 'Content-Type: image/png' -H 'X-Amzn-SageMaker-Custom-Attributes: tfs-model-name=cifar_image' http://localhost:8080/invocations